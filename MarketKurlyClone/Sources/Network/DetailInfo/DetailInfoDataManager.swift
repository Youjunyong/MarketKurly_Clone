...
class DetailInfoDataManager {

    
    func DetailInfoDataManager(delegate: InfoViewController, productId: Int) {
        
//    https://prod.oliviapage.shop/app/users/9/products/5
        let headers: HTTPHeaders = ["X-ACCESS-TOKEN": UserDataManager.shared.token!]
        AF.request("\(Constant.BASE_URL)/app/users/\(UserDataManager.shared.id)/products/\(productId)/detail",
                   method: .get,
                   parameters: nil,
                   encoding: URLEncoding.default,
                   headers: headers)
            .validate()
            .responseDecodable(of: DetailInfoResponse.self) { response in
                ...
                case .success(let response):
                    ...
                    if response.isSuccess  {
                        ...
                        delegate.didSuccessGetInfo(path: result[0].path)
                    }
